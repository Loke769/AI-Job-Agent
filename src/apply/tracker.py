...
def record_application(job: ScoredJob, status: str, resume_path: str = "", cover_path: str = "", cover_letter_path: str = "", receipt_path: str = "", answers: dict = None, error: str = "", jd_text: str = "", gmail_thread_id: str = "", email_status: str = ""):
    p = tracker_path()
    p.parent.mkdir(parents=True, exist_ok=True)
    file_exists = p.exists()
    fieldnames = ["id","company","role","location","url","match_score","status","applied_at","resume_path","cover_letter_path","receipt_path","ats","error","jd_text","gmail_thread_id","email_status"]
    ...
    row = {
        ...
        "status": status,
        "applied_at": now if status in ("applied","failed","queued","approved","skipped","applied (dry-run)","applied (dry-run-playwright)") else "",
        "resume_path": resume_path,
        "cover_letter_path": cover_letter_path or cover_path,
        "receipt_path": receipt_path,
        "ats": job.source,
        "error": error or "",
        "jd_text": (jd_text or job.description or "")[:2000].replace("\n"," ").replace("\r"," "),
        "gmail_thread_id": gmail_thread_id,
        "email_status": email_status,
    }
    # handle existing file with old header — rewrite if needed
...
def save_receipt(job: ScoredJob, resume_path: str, cover_path: str, answers: dict, status: str = "applied") -> pathlib.Path:
    ...
    receipt = {
        ...
        "answers": answers or {},
        "jd": (answers.get("jd") if isinstance(answers, dict) else "") or job.description[:3000],
        "fields_filled": {
            "full_name": "from profile",
            "email": "from profile",
            "phone": "from profile",
            "address": "from profile",
            "work_authorization": "from profile",
            "eeo": "from profile",
            "resume": resume_path,
            "cover_letter": cover_path,
        },
        "gmail": answers.get("gmail") if isinstance(answers, dict) else {},
        "ai_disclosure": "This application was tailored by AI and approved by the candidate. Dry-run receipt — no spam.",
    }
